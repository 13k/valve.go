...
func (s *KeyValueSuite) TestMap() {
	testCases := []kvMapTestCase{
		{
			Name:    "ty_invalid",
			Subject: kv1.NewKeyValueEmpty(),
			Err:     "cannot convert node type Invalid to map",
		},
		{
			Name:    "ty_end",
			Subject: kv1.NewKeyValueEnd(),
			Err:     "cannot convert node type End to map",
		},
		{
			Name:    "non_object",
			Subject: kv1.NewKeyValueString("foo", "bar", nil),
			Err:     "cannot convert node type String to map",
		},
		{
			Name: "object_ok",
			Subject: kv1.
				NewKeyValueRoot("root").
				AddString("str", "foo1").
	...
	for _, testCase := range testCases {
		s.subtestMap(testCase)
	}
}

func (s *KeyValueSuite) subtestMap(testCase kvMapTestCase) {
