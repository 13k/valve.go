...
func (c *Client) post(
	...
	if err != nil {
		return nil, fmt.Errorf("steamweb/web/steam: error encoding form: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("steamweb/steam/web: error performing request: %w", err)
	}

	if !res.IsSuccess() {
		return nil, fmt.Errorf("steamweb/steam/web: HTTP error: %s", res.Status())
	}
...
func (c *Client) Login(credentials *LoginCredentials) (*LoginResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("steamweb/steam/web: error processing credentials: %w", err)
	}
