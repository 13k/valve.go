...
func (r *RSAKeyResult) PubKey() (*rsa.PublicKey, error) {
	if r.pub == nil {
		...
		if err != nil {
			return nil, fmt.Errorf("steamweb/steam/web: error parsing public key: %w", err)
		}
	}
...
func (r *RSAKeyResult) Time() (time.Time, error) {
	if r.ts == nil {
		...
		if err != nil {
			return time.Time{}, fmt.Errorf("steamweb/steam/web: error parsing unix timestamp: %w", err)
		}
