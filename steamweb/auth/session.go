...
func (s *Session) fetchKey() error {
	...
	if !result.Success {
		return fmt.Errorf("steamweb/steam/web: error fetching user key")
	}

	if s.PubKey, err = result.PubKey(); err != nil {
		return fmt.Errorf("steamweb/steam/web: error fetching user key: %w", err)
	}
...
func (s *Session) Login(options *LoginOptions) (SessionState, error) {
	...
	if options.Password == "" {
		return s.State, errors.New("steamweb/steam/web: password is required")
	}
	...
	if err := s.fetchKey(); err != nil {
		return s.State, fmt.Errorf("steamweb/steam/web: error fetching user key: %w", err)
	}
...
func (s *Session) loggedIn(result *LoginResult) error {
	...
	if err = s.genID(); err != nil {
		return fmt.Errorf("steamweb/steam/web: error generating session ID: %w", err)
	}

	if result.TransferParameters != nil {
		if s.SteamID, err = result.TransferParameters.SteamID(); err != nil {
			return fmt.Errorf("steamweb/steam/web: error parsing SteamID: %w", err)
		}
	}

	if s.OAuthResult, err = result.OAuth(); err != nil {
		return fmt.Errorf("steamweb/steam/web: error parsing OAuth result: %w", err)
	}
	...
	for i, transferURL := range result.TransferURLs {
		if mirrors[i], err = url.Parse(transferURL); err != nil {
			return fmt.Errorf("steamweb/steam/web: error parsing transfer URL %q: %w", transferURL, err)
		}
	}
