...
func (s *ClientTestSuite) TestSetCookies() {
	c, err := steamweb.New(steamweb.WithHostURL(s.sv.URL))
...
func (s *ClientTestSuite) TestExecuteResult() {
	c, err := steamweb.New(
		steamweb.WithHostURL(s.sv.URL),
		steamweb.WithHeaders(http.Header{"X-Client": {"client"}}),
		steamweb.WithQueryParams(url.Values{"client": {"client"}}),
	)
