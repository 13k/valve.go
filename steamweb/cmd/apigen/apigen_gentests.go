...
func (g *APIGen) genTestMethod(appIDs []steamlib.AppID, name string, group schema.MethodsIndex) j.Code {
	...
	for _, appID := range appIDs {
		...
		for _, version := range versions {
			...
			if err != nil {
				if _, ok := err.(*schema.MethodNotFoundError); !ok {
					panic(err)
				}
			}
